window.onload = function() {
    ...
    function readFile() {
        ...
        for(var i = 0; i < iLen; i++) {
            ...
            reader.onload = function(e) {
                ...
                result = '<div class="showdiv"><img class="left" src="img/Arrow_left.svg" /><img class="center" src="img/delete.svg" /><img class="right" src="img/Arrow_right.svg" /></div><img id="img' +currentReViewImgIndex+randomString(1)+randomString(2) +randomString(5) + '" class="showimg img-responsive" src="' + this.result + '" />';
    ...
    function send() {
        var dataArr2 = JSON.stringify(dataArr);
        var function1 = $('[name=function]').val();
        var time = $('[name=time]').val();
        var type = $('[name=type]').val();
        var cell = $('[name=cell]').val();
        var mah = $('[name=mah]').val();
        var ww = $('[name=ww]').val();
        var vv = $('[name=vv]').val();
        var hh = $('[name=hh]').val();
        var overtime = $('[name=overtime]').val();
        var sound = $('[name=sound]').val();
        var brush = $('[name=brush]').val();
        var ll = $('[name=ll]').val();
        var kg = $('[name=kg]').val();
        var destory = $('[name=destory]').val();

        fd.append("dataArr2",dataArr2);
        fd.append("function1",function1);
        fd.append("time",time);
        fd.append("type",type);
        fd.append("cell",cell);
        fd.append("mah",mah);
        ...
        fd.append("brush",brush);
        fd.append("ll",ll);
        fd.append("kg",kg);
        fd.append("destory",destory);
